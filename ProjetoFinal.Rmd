...
grafico_skills
```
### Visualização do Rating Médio das empresas com base na sua receita obtida

```{r Visualizacao2}
# Selecionando apenas as colunas necessárias para geração do gráfico.
visualizacao2 <- select(data, Rating, Revenue, `Type of ownership`)
...
grafico_rating_medio <- ggplot(data = conjunto_de_analise) +
                        ...
                        labs(x = "Receita das Empresas", title = "Rating Médio das empresas com base na sua receita Obtida")
...
## Setores que oferecem melhor remuneração para os cientistas de dados que utilizam Python

```{r Visualizacao3}
# # Selecionando apenas as colunas necessárias para geração do gráfico.
setores_e_salarios <- select(data, `Sector`,  `Avg Salary(K)`, `Python`)

plot_ly(x = setores_e_salarios$Sector,
        y = setores_e_salarios$`Avg Salary(K)`,
        z = setores_e_salarios$Python,
        type = "scatter3d")
```